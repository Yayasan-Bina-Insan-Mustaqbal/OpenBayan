...
export async function GET() {
  try {
    ...
    // Fetch relations counts (defines + mentions) in sub-100ms separately
    ...
    // Check Ollama status with 800ms abort timeout
    let ollamaStatus = "offline";
    try {
        const controller = new AbortController();
        const id = setTimeout(() => controller.abort(), 800);
        const res = await fetch("http://100.121.116.17:11434/api/tags", { signal: controller.signal });
        clearTimeout(id);
        if (res.ok) {
            ollamaStatus = "online";
        }
    ...
    return NextResponse.json({ ...metrics, ollamaStatus });
  } catch (error: any) {
