...
def enrich_ayah(text, context):
    ...
    prompt = f"Context: {context}\n\nAnalyze this Ayah: {text}\nMap to ALL relevant categories from this list (provide at least one, multiple if applicable):\n{labels_str}\n\nReturn JSON: {{'categories': [{{'label': '...', 'confidence': 0.9}}], 'entities': [{{'name': '...', 'type': '...'}}]}}"
        ...
        for i in range(total):
            ...
            if enrichment:
                ...
                for cat in enrichment.categories:
                    ...
                    if tag_res:
                        tid = tag_res[0]["id"]
                        db.query(f"RELATE {sid}->tagged_with->{tid} SET weight = $w", {"w": cat.confidence})
                        print(f"  -> Linked to Category: {tag_label} ({cat.confidence})")
