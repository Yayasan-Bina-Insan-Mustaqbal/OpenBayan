...
def setup_graph_and_tag():
    with Surreal(SURREAL_URL) as db:
        ...
        db.query("DELETE category; DELETE tagged_with; DELETE sub_topic;")
        
        # Root: Theology
        res_theo = db.query("CREATE category:theology SET label = 'Theology', category = 'subject';")
        # Child: Attributes of Allah
        res_attr = db.query("CREATE category:attributes SET label = 'Attributes of Allah', category = 'subject', parent = category:theology;")
        # Child: Sovereignty
        res_sov = db.query("CREATE category:sovereignty SET label = 'Sovereignty', category = 'subject', parent = category:theology;")
        
        # Create sub_topic relationships (Edges)
        db.query("RELATE category:attributes->sub_topic->category:theology;")
        db.query("RELATE category:sovereignty->sub_topic->category:theology;")
        ...
        db.query(f"RELATE {sentence_ids[0]}->tagged_with->category:attributes SET weight = 1.0;")
        # Chunk 6: "His Throne extends..." -> Sovereignty
        db.query(f"RELATE {sentence_ids[6]}->tagged_with->category:sovereignty SET weight = 1.0;")
...
def search_with_graph_filter(query_text, tag_id=None):
    ...
    with Surreal(SURREAL_URL) as db:
        ...
        if tag_id:
            ...
            # Find sentences where ->tagged_with->category.id = $tag
            sql = """
                SELECT 
                    text, 
                    vector::similarity::cosine(embedding, $embedding) AS sim,
                    ->tagged_with->category.label AS tags
                FROM sentence 
                WHERE ->tagged_with->category CONTAINS $tag
                ORDER BY sim DESC;
            """
...
def main():
    ...
    res2 = search_with_graph_filter("Attributes of Allah", tag_id="category:sovereignty")
    for r in res2:
        print(f"[Score: {r['sim']:.4f}] Tags: {r['tags']} | {r['text']}")

    # Test 3: Filtered Search (Attributes)
    # This should only return the 'Living/Sustainer' chunk
    res3 = search_with_graph_filter("Attributes of Allah", tag_id="category:attributes")
    for r in res3:
        print(f"[Score: {r['sim']:.4f}] Tags: {r['tags']} | {r['text']}")
