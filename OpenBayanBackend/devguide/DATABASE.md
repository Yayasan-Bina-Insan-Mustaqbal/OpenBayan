...
> **Design Decision — No Embeddings**: Sentence-level embeddings already capture full semantic context. Storing 1024-dim vectors per word across millions of records is not cost-effective and provides no meaningful search advantage.
...
> **`refers_to` Pattern**: When a word is itself a named entity (e.g., the word **`الله`** is a `Divine` entity, **`جبريل`** is a `Celestial` entity), set `refers_to` to point directly to the `entity` record. This creates a direct linguistic-to-ontological bridge without needing a separate relation table.

> **Metadata Enforcement**: Every entry in `translations` and `transliterations` **must** include:
> - `source`: The specific scholarly author or the AI model name (e.g., `"Sahih International"`, `"gemma3:12b"`).
> - `is_ai`: Boolean flag.
